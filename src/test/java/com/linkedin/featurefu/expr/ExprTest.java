...
//        Operator op = OperatorsSupported.getOperator("*");
//        Expr expr = Atom.parse("2",variableRegistry);
//        Expr expr2 = Atom.parse("1",variableRegistry);
//        List<Expr> list = new ArrayList<Expr>();
//        list.add(expr);
//        list.add(expr2);
//        System.out.println( op.calculate(list));
//
//
//        System.out.println(Expression.evaluate("(min 1 (max (+ (* slope x) intercept) 0))"));
//        VariableRegistry variableRegistry=new VariableRegistry();
//        Expr expr =  Expression.parse("(* 2 3)", variableRegistry);
...
//        Assert.assertTrue(Expression.evaluate("(* (if (&& (== 0 12) (&& 3 (&& (&& (>= 4 5) (<= 4 6)) (&& (>= 7 5 ) (<= 7 4))))) 0 (if (&& (== 3 0) (<= 55 3)) 0 (if (&& (== 3 0) (|| (|| (&& 2 (&& 3 1) ) 1) (&& 6 3))) 0 (if (<= 55 12) (/ (* 0.5 55) 12)(+ 0.5 (*(/ 15 1000) (ln (- 55 12)))))))) 1000)") == 1000 * (0.5 + 15.0 / 1000.0 * Math.log(55.0 - 12.0)));
    ...
    public void variables(){
        ...
        Expr expression = Expression.parse("(sigmoid (+ (* sin x) b))", variableRegistry);
        //retrieve variables from variableRegistry by name
        Variable x = variableRegistry.findVariable("x");
        Variable a = variableRegistry.findVariable("sin");
...
//        Assert.assertTrue(expression.evaluate() == 1.0 / (1 + Math.exp(-a.getValue() * x.getValue() - b.getValue())));
    ...
    public void testPrettyPrint(){
        ...
        System.out.println(Expression.prettyTree(expr));
        System.out.println(Expression.evaluate("(+ 0.5 (* (/ 15 1000) (ln (- 55 12))))"));
